...
export default function RouterPage() {
  // ── Uncontrolled refs — typing NEVER triggers re-render ──
  const promptRef = useRef<HTMLTextAreaElement>(null)
  const systemRef = useRef<HTMLTextAreaElement>(null)

  // ── Only these state changes cause re-renders (not typing) ──
  ...
  return (
    <>
      ...
      <div className="flex flex-col md:flex-row md:h-[calc(100vh-108px)]">
        ...
        {/* ── Input Panel ── */}
        <div className="w-full md:w-[40%] md:border-r border-[#E5E2DA] flex flex-col gap-5 p-4 md:p-6 md:overflow-y-auto border-b md:border-b-0">
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              className={`w-full mt-3 font-mono text-[12px] p-3 border border-[#E5E2DA] bg-[#F7F4EE] text-[#0D0D0D] resize-none focus:outline-none focus:border-[#0D0D0D] placeholder-[#8A8A8A] ${showSystem ? 'block' : 'hidden'}`}
            />
          </div>
        ...
        {/* ── Output Panel ── */}
        <div className="flex-1 flex flex-col min-h-[300px] md:min-h-0 md:overflow-hidden">
          {trace && (
            <div className="border-b border-[#E5E2DA] px-4 py-2.5 flex items-center gap-3 bg-[#F7F4EE] flex-wrap shrink-0">
